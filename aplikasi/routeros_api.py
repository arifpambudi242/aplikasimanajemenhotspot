...
class Api:
    ...
    def open_socket(self):
        ...
        try:
            ...
            raise CreateSocketError('Error: API failed to connect to socket. Host: {}, port: {}.'.format(self.address,self.port))
...
if __name__ == '__main__':
    api = Api('192.168.43.25', user='aaa', password='aaa')


    # app = "/ip/hotspot/user/set\n=name=admin1\n?name=admin"
    # app = "/ip/hotspot/user/set\n=.id=2\n=disabled=no"
    # app = "/ip/hotspot/user/print\n?.id=*2"
    # item = 2
    # # app = f"/ip/hotspot/user/set\n=.id={item}\n=disabled=no"
    for x in range(50):
        api.talk(f'/ip/hotspot/user/add\n\
            =name=use{x}\n\
            =profile=Default\n\
            =password=1234')
    print(api.talk(f'/ip/hotspot/user/profile/print'))
        # print(x['shared-users'])
        # print(x['rate-limit'])
    # print(api.talk(f'/user/print\n\
    #     ?name=aaa')[0]['group'])
    # print(api.talk('/ip/hotspot/user/print\n?name=admin7')[0]['name'])
    # for x in api.talk(app):
    # 	print(f" name =  {x['name']}")
    # 	if 'profile' in x:
    # 		print(f"profile = {x['profile']}")
    # print(api.talk('/system/identity/set\n=name=arif'))
    # api.talk('/system/identity/set\n=name=routerADMIN')